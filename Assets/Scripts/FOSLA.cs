...
public class FOSLA : MonoBehaviour, IAI
{
    ...
    public int think(Observer observer, float budget)
    {
        ...
        else if (ThereIsAnEnemy())
        {
            ...
            GameObject enemy = GetClosestRivalEnemy();
    ...
    private GameObject GetClosestRivalEnemy()
    {
        List<EnemyComparer> comparableEnemies = new List<EnemyComparer>();
        _observer.TroopsInField(rival);
        foreach (GameObject enemy in _observer.playersTroops[rival])
        {
            var distance = Math.Abs(Vector3.Distance(enemy.transform.position, myTowerPos));
            var e = new EnemyComparer
            {
                Enemy = enemy,
                Distance = distance
            };
            comparableEnemies.Add(e);
        }
        comparableEnemies.Sort();
